...
export default function ProfilePage() {
  ...
  const verseKeeperProgress = t("verseProgress", {
    current: Math.min(savedVerses.length, 5),
    total: 5,
  })
  const verseKeeperUnlockedUI = hydrated && isVerseKeeperUnlocked
  const verseKeeperProgressUI = hydrated ? verseKeeperProgress : t("verseProgressInitial", { total: 5 })
  ...
  const handleAvatarFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_AVATAR_BYTES) {
      window.alert(t("alertImageTooBig"))
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        window.alert(t("alertAvatarUploadFail"))
        return
      }
  ...
  const saveProfileMutation = useMutation({
    mutationFn: async ({ body }: SaveProfileVars) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(getApiErrorMessage(errData, t("saveFailed")))
      }
  ...
  const resetAppearanceMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}))
        throw new Error(getApiErrorMessage(errData, t("resetAppearanceFailed")))
      }
      return (await res.json()) as AuthUser
    },
    onMutate: async () => {
    ...
    onError: (err, _vars, ctx) => {
      ...
      window.alert(err instanceof Error ? err.message : t("resetAppearanceFailed"))
    },
    onSuccess: (serverUser) => {
      replaceUser(serverUser)
      setThemeBgInput(SUGGESTED_THEME_BACKGROUND)
  ...
  const pushHint = useMemo(() => {
    ...
    if (pushPermission === "default") {
      return t("pushNeedPermission")
    }

    if (pushPermission === "denied") {
      return t("pushBlocked")
    }
    ...
    if (!isPushConfigured) {
      return t("pushServerUnset")
    }

    return hasPushSubscription ? t("pushActive") : t("pushNotConnected")
  }, [hydrated, hasPushSubscription, isPushConfigured, pushPermission, pushSyncErrorMessage, t])
  ...
  const handleSaveProfile = () => {
    ...
    if (!nextNick) {
      window.alert(t("alertNickEmpty"))
      return
    }

    if (!USERNAME_REGEX.test(nextHandle)) {
      window.alert(t("alertUsernameRules"))
      return
    }
    ...
    <section className={styles.profile}>
      <div className={styles.header}>
        <div className={styles.profileHeaderBar}>
          ...
          <div className={styles.headerActions}>
            ...
            <div className={styles.settingsColumn} ref={settingsRef}>
            <button
      ...
      <div className={styles.userInfo}>
        ...
        <button
          ...
          aria-label={avatarPhotoSrc ? t("avatarChange") : t("avatarUpload")}
        >
          <AvatarWithFallback
      ...
      {showProfileEdit ? (
        <div className={styles.profileEdit}>
          ...
          <label className={styles.profileLabel}>
            ...
            >
              <option value="inter">{t("fontInterDefault")}</option>
              <option value="achiko">{t("fontAchiko")}</option>
              <option value="bodoni-moda">{t("fontBodoniModa")}</option>
            </select>
          </label>
          ...
          <div className={styles.profileEditActions}>
            ...
            <button
              ...
              disabled={profileSaving || appearanceSaving}
            >
              {appearanceSaving ? t("resetting") : t("resetAppearance")}
            </button>
          </div>
        </div>
      ) : null}
      ...
      <ul className={styles.list}>
        <li className={`${styles.item} ${styles.itemInteractive}`}>
          ...
          >
            <Image className={styles.imgIcon} src={"/icon-fire.svg"} alt={t("dayStreakAlt")} width={16} height={16} />
            <span>{dayStreak}</span>
            <span className={styles.rewardStatePlaceholder} aria-hidden />
            <p className={styles.statCardLabel}>{t("dayStreak")}</p>
          </Link>
        </li>
        <ProfileChaptersCard />
        <li
      ...
      <div className={styles.savedVersesSection}>
        ...
        {!hydrated || (versesQuery.isPending && savedVerses.length === 0) ? (
          <div className={styles.versesLoadingWrap}>{t("versesLoading")}</div>
        ) : savedVerses.length === 0 ? (
          <div className={styles.empty}>{t("savedVersesEmpty")}</div>
        ) : (
      ...
      {isNotificationsOpen ? (
        <div className={styles.notificationsPanel}>
          <div className={styles.notificationsPanelHeader}>
            <h2 className={styles.notificationsTitle}>{t("notificationsTitle")}</h2>
            <button
              type="button"
              className={styles.notificationsClose}
              onClick={() => setIsNotificationsOpen(false)}
              aria-label={t("notificationsCloseAria")}
            >
              ×
            </button>
          </div>
